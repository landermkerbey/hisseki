...
import { StrokeCharacterData } from "./strokeData";
import { computeStrokeTransform, transformStrokePoint } from "./strokeTransform";
...
export interface StrokeOrderOptions {
  /** Draw a 1-indexed number at each stroke's starting point. Defaults to false. */
  showNumbers?: boolean;
}
 ...
 */
export function drawStrokeOrderCharacter(
  doc: any,
  cell: Cell,
  cellSize: number,
  ...
  if (options.showNumbers) {
    ...
    for (let i = 0; i < data.medians.length; i++) {
      const start = data.medians[i][0] as [number, number];
      const point = transformStrokePoint(start, cellSize, transform);
