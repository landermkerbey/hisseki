...
describe("computeLayout", () => {
    ...
    describe("direction", () => {
        ...
        it("defaults to horizontal (row-major, left-to-right, top-to-bottom) when direction is omitted", () => {
            const result = (0, layout_1.computeLayout)(params);
        ...
        it("is unchanged when direction is explicitly 'horizontal'", () => {
            const withoutDirection = (0, layout_1.computeLayout)(params);
            const withHorizontal = (0, layout_1.computeLayout)({ ...params, direction: "horizontal" });
        ...
        it("traverses column-major, right-to-left, top-to-bottom within each column, for 'vertical' (tategaki order)", () => {
            const result = (0, layout_1.computeLayout)({ ...params, direction: "vertical" });
        ...
        it("does not change the grid's columns/rows counts or cell size, only traversal order", () => {
            const horizontal = (0, layout_1.computeLayout)({ ...params, direction: "horizontal" });
            const vertical = (0, layout_1.computeLayout)({ ...params, direction: "vertical" });
            ...
            expect(new Set(vertical.cells.map((c) => `${c.x},${c.y}`))).toEqual(new Set(horizontal.cells.map((c) => `${c.x},${c.y}`)));
        });
    });
});
