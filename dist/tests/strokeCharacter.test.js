...
const strokeCharacter_1 = require("../src/strokeCharacter");
...
describe("drawStrokeOrderCharacter", () => {
    it("applies the cell's opacity before drawing anything", () => {
        ...
        (0, strokeCharacter_1.drawStrokeOrderCharacter)(mockDoc, { x: 0, y: 0 }, 100, sampleData, 0.4);
    ...
    it("draws each stroke path inside a save/restore block, translated and scaled to fit the cell", () => {
        ...
        const cellSize = 100;
        (0, strokeCharacter_1.drawStrokeOrderCharacter)(mockDoc, cell, cellSize, sampleData, 1);
        ...
        expect(mockDoc.translate).toHaveBeenCalledWith(cell.x + transform.xOffset, cell.y + cellSize - transform.yOffset);
    ...
    it("does not draw stroke-order numbers unless showNumbers is passed", () => {
        const mockDoc = makeMockDoc();
        (0, strokeCharacter_1.drawStrokeOrderCharacter)(mockDoc, { x: 0, y: 0 }, 100, sampleData, 1);
    ...
    it("draws a 1-indexed number at each stroke's starting point when showNumbers is true", () => {
        ...
        const firstPoint = (0, strokeTransform_1.transformStrokePoint)(sampleData.medians[0][0], cellSize, transform);
        const secondPoint = (0, strokeTransform_1.transformStrokePoint)(sampleData.medians[1][0], cellSize, transform);
        expect(mockDoc.text).toHaveBeenNthCalledWith(1, "1", cell.x + firstPoint.x - fontSize / 2, cell.y + firstPoint.y - fontSize / 2, expect.objectContaining({ align: "center" }));
        expect(mockDoc.text).toHaveBeenNthCalledWith(2, "2", cell.x + secondPoint.x - fontSize / 2, cell.y + secondPoint.y - fontSize / 2, expect.objectContaining({ align: "center" }));
    });
    it("draws numbers in red", () => {
        ...
        const redCallOrder = mockDoc.fillColor.mock.invocationCallOrder[mockDoc.fillColor.mock.calls.findIndex((call) => call[0] === "red")];
    ...
    it("draws numbers at full opacity regardless of the character's own (possibly faded) opacity", () => {
        const mockDoc = makeMockDoc();
        (0, strokeCharacter_1.drawStrokeOrderCharacter)(mockDoc, { x: 0, y: 0 }, 100, sampleData, 0.3, {
            showNumbers: true,
        });
        ...
        const opacityOneCallOrder = mockDoc.opacity.mock.invocationCallOrder[mockDoc.opacity.mock.calls.findIndex((call) => call[0] === 1)];
        const firstTextOrder = mockDoc.text.mock.invocationCallOrder[0];
