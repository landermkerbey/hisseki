...
describe("drawCell", () => {
  it("draws the outer border and four guidelines for a cell", () => {
    ...
    drawCell(mockDoc as any, { x: 0, y: 0 }, 72);
  ...
  it("draws a solid, full-opacity border for a second cell drawn after the first", () => {
    ...
    drawCell(mockDoc as any, { x: 72, y: 0 }, 72);
    ...
    const undashBeforeSecondRect = undashCallOrders.some(
      (order) => order > rectCallOrderFirstCell && order < rectCallOrderSecondCell
    );
