...
describe("computeStrokeTransform", () => {
  it("computes scale/offsets for a 300pt cell with 10pt padding (hand-derived reference values)", () => {
    ...
    const transform = computeStrokeTransform(300, 10);
  ...
  it("scales linearly with the cell's effective (padding-excluded) size", () => {
    const small = computeStrokeTransform(100, 10); // effectiveSize = 80
    const large = computeStrokeTransform(200, 20); // effectiveSize = 160, exactly 2x

    expect(large.scale).toBeCloseTo(small.scale * 2, 10);
  });

  it("increases scale as padding shrinks relative to cellSize", () => {
    const tightPadding = computeStrokeTransform(300, 5);
    const loosePadding = computeStrokeTransform(300, 30);
...
describe("transformStrokePoint", () => {
  it("maps CHARACTER_BOUNDS' four corners to within the padded cell area", () => {
    ...
    for (const corner of corners) {
      const point = transformStrokePoint(corner, cellSize, transform);
  ...
  it("maps the raw x-origin (fromX=0) to exactly xOffset", () => {
    const cellSize = 300;
    const transform = computeStrokeTransform(cellSize, 10);

    const point = transformStrokePoint([0, 0], cellSize, transform);
  ...
  it("flips the y axis: larger raw y values map to smaller screen y values", () => {
    ...
    const low = transformStrokePoint([512, 0], cellSize, transform);
    const high = transformStrokePoint([512, 800], cellSize, transform);
  ...
  it("keeps x proportional (no flip) as raw x increases", () => {
    ...
    const left = transformStrokePoint([0, 0], cellSize, transform);
    const right = transformStrokePoint([1024, 0], cellSize, transform);
  ...
  it("maps a roughly-centered raw point near the middle of the cell", () => {
    ...
    const point = transformStrokePoint([512, 388], cellSize, transform);
