...
const generateMock = jest.fn((...args: any[]) => actualGenerate(...args));
jest.mock("../src/generate", () => ({ generate: (...args: any[]) => generateMock(...args) }));
...
function withTmpDir(fn: (tmpDir: string) => Promise<void>): Promise<void> {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "hisseki-test-"));
  return fn(tmpDir).finally(() => fs.rmSync(tmpDir, { recursive: true, force: true }));
}
...
describe("run", () => {
  ...
  it("prints the package version for --version, without requiring --config/--output", () => {
    ...
    expect(io.log).toHaveBeenCalledWith(getVersion(__dirname));
    expect(io.error).not.toHaveBeenCalled();
    expect(io.exit).not.toHaveBeenCalled();
  });
  ...
  it("generates a PDF end-to-end given a valid config and output path", () => {
    return withTmpDir((tmpDir) => {
      ...
      return new Promise<void>((resolve) => {
        run(["--config", configPath, "--output", outputPath], io, () => {
  ...
  describe("--output -", () => {
    it("still reports completion even when the destination stream never fires 'finish' (as real process.stdout/stderr never do when piped)", () => {
      ...
      class NeverFinishesStream extends Writable {
        chunks: Buffer[] = [];
      ...
      return withTmpDir((tmpDir) => {
        ...
        const io = { ...makeIo(), stdout: neverFinishes as unknown as PassThrough };

        return new Promise<void>((resolve) => {
          run(["--config", configPath, "--output", "-"], io, () => {
    ...
    it("writes the PDF to io.stdout instead of a file, and does not require an output path on disk", () => {
      return withTmpDir((tmpDir) => {
        ...
        const io = makeIo();
        const chunks: Buffer[] = [];
        io.stdout.on("data", (chunk: Buffer) => chunks.push(chunk));
  ...
  describe("--dry-run", () => {
    it("validates the config and reports a summary without writing a PDF or requiring --output", () => {
      return withTmpDir((tmpDir) => {
        ...
        const io = makeIo();
        return new Promise<void>((resolve) => {
          run(["--config", configPath, "--dry-run"], io, () => {
            expect(io.error).not.toHaveBeenCalled();
            expect(io.exit).not.toHaveBeenCalled();
    ...
    it("reports an invalid config the same way as a normal run, still without generating", async () => {
      await withTmpDir(async (tmpDir) => {
        const configPath = path.join(tmpDir, "config.json");
        fs.writeFileSync(configPath, JSON.stringify({ pageWidth: -1 }));
    ...
    it("applies CLI overrides before validating/reporting, same as a normal run", () => {
      return withTmpDir((tmpDir) => {
        ...
        const io = makeIo();
        return new Promise<void>((resolve) => {
          run(["--config", configPath, "--dry-run", "--cellSize", "50"], io, () => {
            expect(io.error).not.toHaveBeenCalled();
            expect(io.log).toHaveBeenCalledWith(expect.stringContaining("OK"));
            resolve();
          });
  ...
  describe("--verbose", () => {
    it("logs layout/timing/size details in addition to the normal confirmation, for a real generate run", () => {
      return withTmpDir((tmpDir) => {
        ...
        const io = makeIo();
        return new Promise<void>((resolve) => {
          run(["--config", configPath, "--output", outputPath, "--verbose"], io, () => {
            expect(io.error).not.toHaveBeenCalled();
    ...
    it("logs the applied overrides when any are passed", () => {
      return withTmpDir((tmpDir) => {
        ...
        return new Promise<void>((resolve) => {
          run(
            ["--config", configPath, "--output", outputPath, "--verbose", "--cellSize", "50"],
            io,
            () => {
              const logs = io.log.mock.calls.map((call) => call[0]);
              expect(logs.some((msg) => msg.includes("Overrides:") && msg.includes("cellSize"))).toBe(
                true
              );
              resolve();
            }
    ...
    it("routes ALL diagnostics to stderr instead of stdout when writing the PDF to stdout, so they never corrupt the binary output", () => {
      return withTmpDir((tmpDir) => {
        ...
        return new Promise<void>((resolve) => {
          run(
            ["--config", configPath, "--output", "-", "--verbose", "--cellSize", "50"],
            io,
            () => {
              ...
              expect(errors.some((msg: string) => msg.includes("Overrides:"))).toBe(true);
              expect(errors.some((msg: string) => /Layout:.*grid/.test(msg))).toBe(true);
              expect(errors.some((msg: string) => /Generated in \d+ms/.test(msg))).toBe(true);
              expect(errors.some((msg: string) => msg.includes("Written to stdout"))).toBe(true);
    ...
    it("also enriches --dry-run's summary with overrides when both are passed", () => {
      return withTmpDir((tmpDir) => {
        ...
        return new Promise<void>((resolve) => {
          run(
            ["--config", configPath, "--dry-run", "--verbose", "--cellSize", "50"],
            io,
            () => {
              const logs = io.log.mock.calls.map((call) => call[0]);
              expect(logs.some((msg) => msg.includes("Overrides:") && msg.includes("cellSize"))).toBe(
